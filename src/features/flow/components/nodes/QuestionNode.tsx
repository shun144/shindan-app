...
import { ChoiceSourceHandle } from "@/features/flow/components";
import { useQuestionContextMenu } from "@/features/flow/hooks/useQuestionContextMenu";
import { getNewId } from "@/utils/flowUtils";
import { useAppSelector } from "@/store/store";
import { questionMaxLength, choiceMaxLength, choiceMaxLNum } from "@/features/flow/constants";
import { type Choice, type QuestionNodeType } from "@/features/flow/types";
...
const QuestionNode = ({ id: nodeId, data: nodeData }: NodeProps<Node<QuestionNodeType>>) => {
  ...
  const { showQuestionContextMenu } = useQuestionContextMenu();
    ...
    (event: ChangeEvent<HTMLInputElement>, _choice: Choice) => {
      ...
      updateNodeData(nodeId, {
        ...
        choices: nodeData.choices.map((c) =>
          c.id === _choice.id ? { ...c, content: targetValue } : c
        ),
      });
    },
  ...
  const handleDeleteChoice = useCallback(
    ...
    [updateNodeData, nodeId, nodeData, setEdges]
  );

  function handleClickSubmenu(event: React.MouseEvent<SVGElement, MouseEvent>) {
    showQuestionContextMenu(event, nodeId);
  }

  return (
    <div
      className={`rounded-md w-96 bg-slate-900 shadow-cyan-100/50 relative  ${
        firstNodeId === nodeId ? " border-yellow-300 border-4" : " border-slate-300 border-2"
      }`}
    >
      {firstNodeId === nodeId && (
      ...
      <div className="flex flex-col justify-center items-center cursor-default pt-3 pb-6 px-3">
        <div className="w-full flex flex-col justify-center items-center relative">
          <label
            htmlFor="message"
            className="self-start block text-md font-semibold text-indigo-500"
          >{`質 問（${questionMaxLength}文字）`}</label>
          <textarea
            id="message"
            rows={3}
        ...
        <div className="w-full flex flex-col justify-center items-center pt-6">
          ...
          {nodeData.choices.map((choice, idx) => (
            <div
              key={choice.id}
              className={`w-full relative ${idx != nodeData.choices.length - 1 && "pb-6"}`}
            >
              <div className="w-full flex justify-between">
                <input
                  className="w-full text-sm hover:shadow text-slate-200  placeholder-slate-500 bg-slate-800 rounded-sm border-1 ring-0 border-slate-400 focus:ring-0 focus:border-slate-200"

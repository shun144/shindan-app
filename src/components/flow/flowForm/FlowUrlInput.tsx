...
const FlowUrlInput = () => {
  ...
  return (
    <div className="w-full flex justify-start items-center pr-4">
      <label htmlFor="url" className="w-2/12 block select-none  text-base font-medium text-gray-300 h-9 leading-9 text-center">
        URL
      </label>
      <input type="text" ref={refUrl} id="url" value={flowUrl} onChange={handleChangeUrl} placeholder="URLを入力してください" className="w-9/12 bg-white border border-slate-400 text-gray-900 text-sm focus:ring-transparent focus:border-blue-400 block  p-2.5 h-9 placeholder-slate-400 rounded" required />

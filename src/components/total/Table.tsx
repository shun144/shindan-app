...
import DebouncedInput from "@/parts/Input/DebouncedInput";
        ...
        <div className="bg-white overflow-hidden shadow-sm">
          <div className="pb-3 px-6">
                ...
                {table.getRowModel().rows?.length ? (
                  table.getRowModel().rows.map((row) => (
                    <Fragment key={row.id}>
                      ...
                      {/* グラフ表示 */}
                      <AnimatePresence>{expandedRows[row.index] && <ExpandableRow key={`graph_${row.id}`} flowId={row.original.id} colLength={columns.length} />}</AnimatePresence>
                    </Fragment>
                  ))
                ) : (
                  <tr>
            ...
            <div className="flex items-center justify-center mt-4 gap-8">
              ...
              <select value={table.getState().pagination.pageSize} onChange={(e) => table.setPageSize(Number(e.currentTarget.value))} className="py-2 pr-8 bg-transparent cursor-pointer select-none border-1 border-slate-200 focus:ring-0 focus:border-blue-500 text-sm">
